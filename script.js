...
document.querySelectorAll(".vinilo").forEach(btn => {
  btn.addEventListener("click", async () => {
    // Necesario para habilitar audio en navegadores
    await Tone.start();

    // Crear un sintetizador y enviar el sonido a la salida
    const synth = new Tone.Synth().connect(Tone.Destination);

    // Reproducir un sonido breve
    synth.triggerAttackRelease("C4", "8n");

    // Seleccionar el video correspondiente
    let videoURL = "";
    if (btn.textContent.includes("Proyectos")) {
      videoURL = "https://www.youtube.com/embed/XXXXXXXX";
    } else if (btn.textContent.includes("Composiciones")) {
      videoURL = "https://www.youtube.com/embed/YYYYYYYY";
    } else if (btn.textContent.includes("Colaboraciones")) {
